      ...
      >
        ...
        >
          {collapsed ? (
            ...
            <Image src="/logo-sim-blanc.svg" alt="SIM Assurances" width={188} height={28} priority />
          )}
          <button
            type="button"
            onClick={onCloseMobile}
...
/**
 * Icône seule du logo (triangle stylisé "SA"), pour la sidebar réduite.
 * Reprend exactement la géométrie de `public/logo-sim-couleur.svg`/
 * `logo-sim-blanc.svg` (voir CLAUDE.md) — remplace un précédent triangle
 * générique de substitution qui ne correspondait pas au vrai logo.
 * `fill="currentColor"` : couleur pilotée par la classe `text-*` appelante
 * (une seule teinte ici, pas le bicolore de la version complète).
 */
